...
void Game :: advance()
{
   if (lander.isAlive() && !lander.isLanded())
   {
      ...
      smallAsteroid.applyGravity(GRAVITY_AMOUNT);
      smallAsteroid.advance();
      ...
      // check for crash into asteroid
      if (lander.getPoint().getX() >= smallAsteroid.getPoint().getX() - 18 &&
          lander.getPoint().getX() <= smallAsteroid.getPoint().getX() + 18 &&
          lander.getPoint().getY() >= smallAsteroid.getPoint().getY() - 18 &&
          lander.getPoint().getY() <= smallAsteroid.getPoint().getY() + 18)
      {
         lander.setAlive(false);
      }

      //check to see if asteroid crashed
      if (!ground.isAboveGround(smallAsteroid.getPoint()))
      {
