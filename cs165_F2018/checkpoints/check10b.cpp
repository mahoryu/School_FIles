...
void promptInventory(vector<Food*> &items)
{
   ...
   do
   {
      ...
      if (name != "quit")
      {
         ...
         Food * pFood;
         pFood = new Food(name, month, year);
         items.push_back(pFood);
         cout << endl;
      }
                        ...
                        int currentYear)
{
   ...
   for (vector <Food*> :: iterator it = items.begin(); it != items.end(); /*blank*/)
   {
      Food* pItem;
      pItem = *it;
      
      if (pItem->getYear() < currentYear)
      {
         it = items.erase(it);
         delete pItem;
         pItem = NULL;
      ...
      else if(pItem->getYear() == currentYear &&
              pItem->getMonth() < currentMonth)
      {
         it = items.erase(it);
         delete pItem;
         pItem = NULL;
